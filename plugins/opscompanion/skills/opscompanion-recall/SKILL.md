...
> **Rate limiting decision** (captured by kenneth on Feb 28):
> The team chose sliding-window rate limiting with a Redis backend over token bucket, because it handles bursts better for your use case. Config lives in config-service, not hardcoded. Default limits: 1000 req/min authenticated, 100 req/min anonymous.
